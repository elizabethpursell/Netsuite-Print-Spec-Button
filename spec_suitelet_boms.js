define(['N/record', 'N/search', 'N/runtime', 'N/redirect'], function(record, search, runtime, redirect){
    ...
    function onRequest(context) {
        var searchAssembliesString = context.request.parameters.searchAssembliesString;             //gets parameters from assembly suitelet
        const searchAssemblies = JSON.parse(searchAssembliesString);
        var index = 0;
        var overflow = false;
        const bomArray = [];            //creates array of internal ids of all boms without assemblies
        ...
        var currentAssembly = parseInt(searchAssemblies[0]);
